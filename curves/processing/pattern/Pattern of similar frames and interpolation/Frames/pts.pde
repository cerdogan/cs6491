...
class pts {
 ...
 pt[] G = new pt [maxnv];                 // geometry table (vertices)
  ...
  pts roundOff() {for (int i=0; i<nv; i++) G[i].roundOff(); return this;}
  ...
  pts draw(float r) {for(int v=0; v<nv; v++) show(G[v],r); return this;}
  pts draw() {for (int v=0; v<nv; v++) show(G[v],13); return this;}
  pts drawCurve() {beginShape(); for (int v=0; v<nv; v++) v(G[v]); endShape(); return this;}
  ...
  pts dragPicked() {   // moves selected point (and next if p even) by amount mouse moved recently
      G[pv].moveWithMouse(); 
