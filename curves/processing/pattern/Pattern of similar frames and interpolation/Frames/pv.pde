...
class pt { float x=0,y=0; 
  ...
  pt v() {vertex(x,y); return this;};                                                                     // vertex for drawing polygons between beginShape() and endShape()
  pt v(float u, float v) {vertex(x,y,u,v); return this;}                                                                     // vertex for drawing polygons between beginShape() and endShape()
  pt roundOff() {x=round(x); y=round(y); return this;}                                                                     // vertex for drawing polygons between beginShape() and endShape()
...
vec R(vec V, float a) {float c=cos(a), s=sin(a); return(V(V.x*c-V.y*s,V.x*s+V.y*c)); };                                     // V rotated by a radians
...
// text
String coordString(pt P) {return "("+nf(P.x,1,4)+","+nf(P.y,1,4)+")";}
